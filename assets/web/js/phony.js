var run = function () {
  var fetchVersions = function () {
    ...
    request.onload = function () {
      ...
      latest.setAttribute("href", "../latest/" + page + window.location.hash);
  ...
  var dispatch = function (event) {
    ...
    if (window.location.hash) {
      ...
      if (target && target.classList.contains("anchor")) {
        document.title = target.parentNode.innerText + " - " + documentTitle;
      }
      ...
      if (target) {
        ...
        if (matches) {
          if ("facade" === matches[1]) {
            ...
            document.title =
              "$" + matches[1] + "->" + matches[2] + "() - " + documentTitle;
          }
        }
